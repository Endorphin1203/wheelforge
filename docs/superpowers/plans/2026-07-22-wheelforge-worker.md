...
**Architecture:** A database consumer leases `READY` rows from MySQL with `SELECT ... FOR UPDATE SKIP LOCKED`, then runs focused pipeline stages against a rooted local data directory. pip supplies supported cross-target resolution reports; `packaging` independently parses requirements and validates versions, markers, and Wheel tags; all commands use argument arrays through a restricted process runner.

**Tech Stack:** Python 3.12, pip 26.1.x, packaging, Pydantic 2, SQLAlchemy 2, PyMySQL, pathlib/local filesystem storage, charset-normalizer, Jinja2, pytest.
...
- V1 never connects to Redis, MinIO, S3, or a Docker socket; jobs, cancellation, leases, and retries use MySQL.
- Persistent object keys are generated by the system and must resolve beneath the configured local data root.
...
```python
def test_only_one_worker_leases_a_ready_job(repository, ready_job) -> None:
    first = repository.claim_next("worker-a")
    second = repository.claim_next("worker-b")
    assert first.id == ready_job.id
...
- [ ] **Step 3: Implement leased database consumption and stage orchestration**

Poll MySQL at the configured interval. In a short transaction select one due `READY` job ordered by priority and creation time using `FOR UPDATE SKIP LOCKED`, then set `RUNNING`, a fresh execution ID, lease owner, lease expiry, heartbeat, and incremented attempt count before committing. Renew the lease between stages; reclaim expired `RUNNING` jobs only while attempts remain, otherwise mark them `FAILED`. Validate the version-1 JSON payload before dispatch by `job_type`.

For `REQUIREMENT_PARSE`, atomically change the owned RequirementFile from `PENDING` to `PARSING`, read the original key through a root-constrained local storage helper, call `parse_requirements`, atomically write the UTF-8 normalized object, insert RequirementItem rows, and set `PARSED` or `FAILED`. For `BUILD`, conditionally claim the `QUEUED` BuildTask with the job execution ID and `version_no`, append monotonic logs, and invoke resolution/download/static-validation/package stages. Poll the MySQL cancellation flag at every boundary. Build and hash the ZIP in the workspace, stream-copy it to a temporary file inside the data root, verify the copied hash, atomically rename that temporary file to the generated Artifact key on the same filesystem, then publish Artifact metadata; delete the unreferenced file if the database transaction fails. Map complete static validation to `SUCCESS`, useful incomplete output to `PARTIAL_SUCCESS`, unrecoverable errors to `FAILED`, and requested cancellation to `CANCELLED`. Mark the job terminal in the same transaction as its subject's terminal update where possible. No stage accesses Redis, MinIO, Docker, a target container, or a virtual environment, and no stage executes dependency code.
